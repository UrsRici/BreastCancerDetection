...
namespace Licenta_Mamograf
{
    ...
    public class Clahe
    {
        ...
        private int[] computeHistogram(int x0, int x1, int y0, int y1)
        {
        ...
        private int[] clipContrast(int[] histo)
        {
        ...
        private int transformPixelIntensity(int x, int y, int binX, int binY, int pixVal)
        {
            float val = pixVal;
        ...
        private float bilinear_interpolation(int x, int y, int width, int height, int v1, int v2, int v3, int v4)
        {
            float val1, val2; //intermediate vals.
            float res;

            if (x < 0 || y < 0)
                return 0; // debug, this should never happen
